...
pub fn run(
    ...
    if !io::stdin().is_terminal() || !io::stdout().is_terminal() {
        bail!("interactive mode needs a terminal; use `tonantzintla-installer dry-run`");
    }
...
fn render_welcome(frame: &mut ratatui::Frame, area: Rect, source: &std::path::Path) {
    ...
    let copy = vec![
        Line::from(Span::styled(
            "DEPLOY TONANTZINTLA WITHOUT GUESSWORK",
            Style::default().fg(ACCENT).add_modifier(Modifier::BOLD),
        )),
        Line::raw(""),
        ...
        Line::from(vec![
            ...
            Span::raw(
                source
                    ...
                    .unwrap_or_else(|| "Tonantzintla checkout".to_string()),
            ),
        ]),
        Line::from(vec![
