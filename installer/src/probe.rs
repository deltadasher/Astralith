...
impl ProbeContext {
    pub fn from_process() -> Result<Self> {
        ...
        let os_release = env::var_os("ASTRALITH_INSTALLER_OS_RELEASE")
            .map(PathBuf::from)
    ...
    fn rooted_absolute(&self, path: &Path) -> Option<PathBuf> {
        let root = env::var_os("ASTRALITH_INSTALLER_ROOT").map(PathBuf::from)?;
    ...
    let mut processes = child_paths(&proc_root)
        ...
        .filter_map(|path| {
            ...
            let command_line = fs::read(path.join("cmdline"))
                ...
                .unwrap_or_default();
            let astralith_owned = command_line.to_ascii_lowercase().contains("astralith");
            Some(DesktopProcess {
                pid,
                name,
                role: role.to_string(),
