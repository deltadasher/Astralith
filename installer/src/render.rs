...
pub fn report_text(report: &MachineReport) -> String {
    ...
    let _ = writeln!(out, "ASTRALITH // MACHINE REPORT");
    ...
    for process in &report.desktop_processes {
        let owner = if process.astralith_owned {
            "Astralith"
        } else {
            "external"
        };
...
fn plan_text_with_mode(plan: &InstallPlan, dry_run: bool) -> String {
    let mut out = String::new();
    if dry_run {
        let _ = writeln!(out, "ASTRALITH // DRY RUN");
        let _ = writeln!(
            out,
            "No packages, files, services, or settings will be changed.\n"
        );
    } else {
        let _ = writeln!(out, "ASTRALITH // EXECUTION PLAN");
        let _ = writeln!(
            out,
