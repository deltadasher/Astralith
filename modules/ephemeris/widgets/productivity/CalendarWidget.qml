...
Item {
    ...
    Item {
        ...
        Item {
            ...
            Repeater {
                ...
                Item {
                    ...
                    readonly property real bodySize: index < 4 ? 9 : index < 6 ? 14 : 11
                    ...
                    Rectangle {
                        ...
                        anchors.centerIn: parent
                        width: parent.width + 10; height: 4; radius: 2
                        color: "#b9a778"
                        opacity: 0.48; rotation: -18; z: -1
                    }
                    Text {
        ...
        Item {
            ...
            Canvas {
                ...
                function repaintGlobe() { requestPaint(); }
                onWidthChanged: requestPaint()
                onHeightChanged: requestPaint()
                onPaint: {
                    const ctx = getContext("2d");
        ...
        Canvas {
            ...
            function drawRimMarker(ctx, degrees, innerRadius, outerRadius, thickness, tone, bend) {
                ...
                const normalX = -Math.sin(angle) * bend;
                const normalY = Math.cos(angle) * bend;
                ctx.strokeStyle = tone;
                ctx.lineWidth = thickness;
                ctx.lineCap = "round";
                ctx.beginPath();
                ctx.moveTo(sx, sy);
                ctx.quadraticCurveTo((sx + ex) / 2 + normalX,
                    (sy + ey) / 2 + normalY, ex, ey);
                ctx.stroke();
                ctx.fillStyle = tone;
                ctx.beginPath(); ctx.arc(ex, ey, thickness * 0.62, 0, Math.PI * 2); ctx.fill();
            }
            ...
            onPaint: {
                ...
                drawRimMarker(ctx, root.hours * 30, width * 0.425, width * 0.468, 7, Theme.cyan, pulse * 2.2);
                drawRimMarker(ctx, root.minutes * 6, width * 0.442, width * 0.482, 4.5, Theme.accent, -pulse * 2.8);
                drawRimMarker(ctx, root.seconds * 6, width * 0.458, width * 0.492, 2, Theme.rose, pulse * 3.2);
            }
            Connections {
                target: root
                enabled: root.calendarMode === 0
    ...
    Item {
        ...
        Item {
            id: forecastConstellation
            x: complications.width * 0.02 + (1 - root.sideThree) * 250
                + (1 - root.calendarSupportReveal) * 92
            y: complications.height * 0.18
            width: complications.width * 0.96; height: 450
            ...
            Repeater {
                ...
                Item {
                    ...
                    required property int index
                    required property var modelData
                    x: forecastConstellation.pointX(index) - width / 2
                    y: forecastConstellation.pointY(index) - height / 2
                    width: 280; height: 62
                    z: 3
                    transform: Translate { id: forecastDrift }

                    Rectangle {
                        anchors.fill: parent; radius: height / 2
                        color: forecastDayPointer.containsMouse ? Theme.accent : Theme.controlHover
                    }
                    Row {
                        anchors.centerIn: parent; spacing: 16
                        Text {
                            text: dayNode.modelData.day
                            color: forecastDayPointer.containsMouse ? Theme.void_ : Theme.moon
                            font.family: Theme.fontMono; font.pixelSize: 13; font.weight: Font.Black
                        }
                        Text {
                            text: dayNode.modelData.icon
                            color: forecastDayPointer.containsMouse ? Theme.void_ : Theme.cyan
                            font.family: Theme.fontDisplay; font.pixelSize: 21
                        }
                        Column {
                            anchors.verticalCenter: parent.verticalCenter; spacing: 1
                            Text {
                                width: 90; text: dayNode.modelData.condition.toUpperCase()
                                color: forecastDayPointer.containsMouse ? Theme.void_ : Theme.moon
                                font.family: Theme.fontMono; font.pixelSize: 11; font.weight: Font.Bold
                                elide: Text.ElideRight
                            }
                            Text {
                                text: dayNode.modelData.precipitation + "% PRECIP"
                                color: forecastDayPointer.containsMouse ? Theme.void_ : Theme.muted
                                font.family: Theme.fontMono; font.pixelSize: 11; font.weight: Font.Bold
                            }
                        }
                        Text {
                            anchors.verticalCenter: parent.verticalCenter
                            text: root.temperature(dayNode.modelData.max) + " / " + root.temperature(dayNode.modelData.min)
                            color: forecastDayPointer.containsMouse ? Theme.void_ : Theme.accent
                            font.family: Theme.fontMono; font.pixelSize: 12; font.weight: Font.Bold
                        }
                    }
                    MouseArea {
                        id: forecastDayPointer
                        anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                        onClicked: root.stepDays(dayNode.index)
                    }
                    SequentialAnimation {
                        loops: Animation.Infinite
                        running: Settings.motion && forecastConstellation.visible && root.sideThree > 0.99
                        PauseAnimation { duration: dayNode.index * 110 }
                        NumberAnimation { target: forecastDrift; property: "x"; to: -4; duration: 1300; easing.type: Easing.InOutSine }
                        NumberAnimation { target: forecastDrift; property: "x"; to: 4; duration: 1900; easing.type: Easing.InOutSine }
                        NumberAnimation { target: forecastDrift; property: "x"; to: 0; duration: 1200; easing.type: Easing.InOutSine }
                    }
                }
            }
    ...
    Row {
        ...
        Repeater {
            ...
            Rectangle {
                ...
                height: 32; radius: height / 2
                color: root.calendarMode === modeButton.modelData.mode
                    ? Theme.accent : modePointer.containsMouse ? Theme.controlHover : Theme.controlRest
                Text {
                    anchors.centerIn: parent; text: modeButton.modelData.label
                    color: root.calendarMode === modeButton.modelData.mode ? Theme.void_ : Theme.moon
                    font.family: Theme.fontMono; font.pixelSize: 10; font.weight: Font.Black
                }
                MouseArea {
                    id: modePointer
                    anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                    onClicked: root.setCalendarMode(modeButton.modelData.mode)
                }
                Behavior on color { ColorAnimation { duration: Settings.motion ? 160 : 0 } }
            }
        }
    }
