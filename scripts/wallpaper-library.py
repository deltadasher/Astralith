#!/usr/bin/env python3
"""Index local image/video wallpapers with cached Serpantinum metadata."""
...
def video_preview(path: Path) -> str:
    thumb_dir = cache_home() / "astralith" / "wallpaper-thumbs"
...
def source_name(path: Path, bundled_root: Path) -> str:
    ...
    if value.startswith("/usr/share/backgrounds"):
        return "System"
    if path.name.startswith(("ddg_", "astralith-")):
        return "Downloaded"
    return "Library"
...
def main() -> None:
    ...
    # Keep Astralith's bundled worlds and video wallpapers discoverable even
